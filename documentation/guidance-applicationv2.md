...
This document describes how to build Application V2 windows that modules (e.g. Blacksmith consumers) can use or replicate. It is based on the Artificer Skills Window pattern: header + scrollable body (optionally with a side panel) + action bar. The goal is a reusable window API pattern and a plug-in example framework.
...
### 1.3 Template Structure (Skills-Window Pattern)

Use a single root element with a stable class so the app can find its DOM after render:

- **Header** — flex row: icon/title block on the left, optional toggles/values on the right.
- **Body** — scrollable main area; optionally a two-column layout (e.g. panels left, details right).
- **Action bar** — fixed at bottom: secondary button(s) left, primary button(s) right.

Example skeleton:

```html
<div class="my-window-root">
...
- **Optional base class:** A thin “WindowWithHeaderAndActions” base class could encapsulate PARTS template path, default structure (header + body + buttons), `_getRoot()`, scroll save/restore, and delegation wiring so each module only supplies `getData`, actions, and optional CSS.

---

## 6. Example: Minimal Window Framework

Below is a **minimal plug-in window**: same layout as the Skills window (header, body, optional details area, action bar) but with **no content** in the header or body. Copy this into your module and replace placeholders with your module id, template path, and styles.

**Copyable files** are in `documentation/example-applicationv2-window/` (template, script, and README).

### 6.1 Template

```handlebars
{{!-- Minimal Application V2 window - header + body + action bar. No content; replace with your own. --}}
<div class="example-window-root" id="{{appId}}">
    ...
    <div class="example-window-body">
        <div class="example-window-main">
            {{!-- Main content area (e.g. list or form) --}}
...
- Header: `flex-shrink: 0`; body: `flex: 1; overflow: auto`.
- Body can use a two-column flex (e.g. `.example-window-main` + `.example-window-details`) with the details panel having a fixed or min width and scrollable content.
- Buttons section: `flex-shrink: 0` at the bottom.

Reference `styles/window-skills.css` in this repository for a full layout (header, panels, details, scroll, action bar).
...
| Layout | Single root (e.g. `my-window-root`) with header, body, action bar |
