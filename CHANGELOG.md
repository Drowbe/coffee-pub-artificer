...
### Fixed
- **Full hook coverage for external pin deletions:** `blacksmith.pins.deleted` fires for all single-pin deletions (right-click, Manage Pins, `pins.delete()` API) and includes `pinId` in the payload — Artificer now extracts it and removes the matching `discoveredNode` directly rather than relying on a list comparison. Bulk deletions (`pins.deleteAll()` / `pins.deleteAllByType()`) fire separate hooks (`blacksmith.pins.deletedAll`, `blacksmith.pins.deletedAllByType`) with no per-pin IDs — those are handled via a live `pins.list()` reconcile. All three hooks update scene flags on the GM side and reload the pin layer for the active canvas scene.
...
- **Gathering spot images randomising on every sync:** `resolveGatheringImageForScene` uses `Math.random()` to pick from an image pool, so every call to `syncScenePins` produced a different image for every existing pin — causing all pins to visually refresh whenever any scene flag changed (harvest, populate, etc.). Fixed by: (1) storing the chosen `idleImage` in the node data at creation time in both the populate and discovery flows so the image is committed once; (2) changing the update-path image check in `syncScenePins` to use `node.idleImage || pin.image` instead of re-calling the random resolver — existing pins keep their current image unless the node explicitly carries a different one.
